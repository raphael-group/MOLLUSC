...
def get_tree(file_name):
	...
	#newick_tree = normalize_tree(newick_tree)
	f.close()
	return newick_tree
...
def initialize_internals(length):
	...
	x_vec = np.random.normal(x_avg, current_sigma, length)
	y_vec = np.random.normal(y_avg, current_sigma, length)
	initial = []
...
def get_multivariate_normal_nllh(internal_locations):
	x_mean_vec, y_mean_vec = construct_mean_vector(internal_locations)
	...
	for node in current_tree.traverse_postorder():
		if node.is_leaf():
			...
			y_index = x_index + 1
			all_locations_x.append(internal_locations[x_index])
			all_locations_y.append(internal_locations[y_index])
...
def construct_covariance_matrix():
	...
	for node_i in current_tree.traverse_postorder():
		...
		for node_j in current_tree.traverse_postorder():
			...
			entry[j_index] = current_sigma**2 * dist_from_root(lca)
			j_index += 1
	...
	with open(name_to_save,'w') as fout: 
		for node in current_tree.traverse_postorder():
			if node.is_leaf():
				...
				x_index = internal_mapping[node.label]
				y_index = x_index + 1
				x_loc = internal_locations[x_index]
				y_loc = internal_locations[y_index]
...
methods_to_compare = ["spalin-divide-beta5", "spalin" ]
